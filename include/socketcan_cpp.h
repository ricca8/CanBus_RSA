...
#include <string>
#include "../include/socketcan_cpp_export.h"
...
namespace scpp{
    ...
    //Two types of Can, first not used
    enum SocketMode{
        MODE_CAN_MTU = CAN_MTU,
        MODE_CANFD_MTU = CANFD_MTU
    };

    //Data has 64 slots because we are using canfd
    struct CanFrame{
        int id = 0;
        char len = 0;
        char flags = 0;
   ...
   class SocketCan{
    public:
        ...
        SOCKETCAN_CPP_EXPORT SocketCanStatus open(const std::string & can_interface, int read_timeout_ms = 3, SocketMode mode = MODE_CANFD_MTU);
